...
static int gp_mode_set(int mode)
{
	...
	/*
	if (video_down_flag == 1)
	{
		while(1)
		{
			OSTimeDly(1);
			if (video_down_flag == 0) break;
		}
	}
	...
	*/
	//DBG_PRINT("\r\n=======mode:%d;  mode_cur=%d;  mode_bak:%d=======\r\n", mode,mode_cur,mode_bak);
...
static int General_GetParameterFile_Decode(unsigned short gp_sock_cmd, socket_cmd_ctl_t *ctl_blk, INT8U* rxstring)
{
	...
	if (ui_language != 1)
		xml_fd = nv_open((INT8U *) "WIFI_EN.XML");		//英文
	else
		xml_fd = nv_open((INT8U *) "WIFI_CN.XML");		//中文
	...
	*pGP_ID++ = p[0];		// CMD_ID
	...
	while (xml_len > 0)
	{
		...
		xml_len -= (SOCKET_CMD_BUF_SIZE-GP_SOCK_RESP_PAYLOAD_OFFSET);			//data segment
		if (xml_len > 0)
		{
...
*/
	...
	if(Wifi_State_Get() == WIFI_STATE_FLAG_CONNECT)
	{
		...
		else		// record stop
		{
			...
			{
				OSTimeDly(50);
				if ( (video_record_sts & VIDEO_RECORD_BUSY) == 0)		//wait video record stop
				{
					break;
				}
			}
...
static int gp_cmd_process(socket_cmd_ctl_t *ctl_blk, INT8U* rxstring)
{
	...
	case GPSOCK_Record_Audio:
		...
		break;
	case GPSOCK_CapturePicture_Capture:
...
void socket_cmd_task(void *pvParameters)
{
	...
	for(;; )
	{
		...
		if (err == SC_RET_SUCCESS)
		{
			printf("socket_cmd_task: A new client connected\r\n");
