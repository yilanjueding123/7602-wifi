...
void set_led_mode(LED_MODE_ENUM mode)
{
	...
	switch((INT32U)mode)
	{
		...
		case LED_WAITING_CAPTURE://拍照待机
			...
			//G_PRINT("g_wifi_notify_pic_done = %X\r\n", g_wifi_notify_pic_done);
			break;
		case LED_CAPTURE://拍照
			led_red_off();
			//led_blue_off();
		...
		case LED_RECORD_IR_ENABLE:
			g_led_r_state = 2;
			g_led_flicker_state = 0;
			DBG_PRINT("led_type = LED_RECORD_IR_ENABLE\r\n");
			break;
		...
		case LED_WIFI_ENABLE:
			...
			if (led_blue_flag == LED_ON)
				LedFilkerStep = BLUE_FLIKER|0x01;
			else
				LedFilkerStep = BLUE_FLIKER;
...
void  video_busy_cnt_clr(void)					//inline function will be better than call funtion.
{
	video_busy_cnt = 0;
}
...
*/
void  wifi_connected_state(INT8U flag)
{
	wifi_connected_ok = flag;
}

INT8U wifi_connected_state_get(void)
{
	return wifi_connected_ok;
}

extern volatile INT8U wifi_init_sta;
void wifi_connect_led_fliker(void)
{
	static INT8U nFlag = 0;
	nFlag ^= 1;
	if (nFlag)
	{
		led_wifi_on();
...
void wifi_led(void)
{
	...
	static INT32U SecCnt = 0;
	
	if (wifi_init_sta) return;
	
	if (wifi_connected_ok)
	{
	/*
		if (++SecCnt > 32) 
		{
					...
					if ((screen_auto_off != 0) && !auto_off_force_disable && (usb_state_get()==0) && !md_status&!wifi_key_snd_flag) { //don't auto off under following conditions:
						...
						*/
						...
						if (cnt_sec > screen_auto_off*60) {
							key_active_cnt = 0;
							type = 0;
							msgQSend(ApQ, MSG_APQ_POWER_KEY_ACTIVE, &type, NULL, MSG_PRI_NORMAL);
