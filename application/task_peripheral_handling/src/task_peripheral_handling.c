	...
	while(1) {
        ...
        switch (msg_id) {
			...
			case MSG_PERIPHERAL_TASK_BATTERY_LOW_BLINK_START:
				...
				battery_low_blink_cnt |= 0x00800;//闪烁次数,  60次,   120/2
				break;
        	...
        	case MSG_PERIPHERAL_TASK_AD_DETECT_CHECK:
        		...
        		if(battery_low_blink_cnt){
        			...
        			if(!(battery_low_blink_cnt&0x00ff)){
         				if(battery_low_blink_cnt&0xff00){
         					...
         					type = 1;
							msgQSend(ApQ, MSG_APQ_POWER_KEY_ACTIVE, &type, NULL, MSG_PRI_URGENT);
        					battery_low_blink_cnt = 0;
         					break;
         				}
        		...
        		if(G_sensor_power_on_time){
					...
					if(G_sensor_power_on_time<(0x300-0x80)){ //check SDC
	       				if(adp_status !=1){
	       				/*
							if(ap_state_handling_storage_id_get()!=MINI_DVR_STORAGE_TYPE){
 	     		...
 	     		//	DBG_PRINT("adp_status = %d\r\n",adp_status);
        			if(!G_sensor_power_on_time)
        			{
	       				if(adp_status !=1){		//check adptor
	       				/*
