...
#define JAN_31TH_2015        2457054
#define OCT_01TH_2016        2457663 //2016-10-01

INT32U LAST_JULIAN_DATE = OCT_01TH_2016;
...
//static void cal_day_calculate(INT32U day);
...
static INT32U cal_day_get(void)
{
	...
	if(r_time.rtc_day == 0) {
		TIME_T tm;
		//LAST_JULIAN_DATE = JAN_31TH_2015;
		DBG_PRINT("D=%d  H=%d  M=%d S=%d setup\r\n",r_time.rtc_day,r_time.rtc_hour,r_time.rtc_min,r_time.rtc_sec);
        	tm.tm_sec = r_time.rtc_sec;
    	   	tm.tm_min = r_time.rtc_min;
    	   	tm.tm_hour = r_time.rtc_hour;
    	   	tm.tm_year = 2016;//2008;
   	   	...
   	   	tm.tm_mday = 1;//8;
   	   	tm.tm_wday = 6;
		cal_time_set(tm);	// 燒錄後第一次問機▼N機，經過 24 小時，天數沒有 +1		
	}
	else {
		DBG_PRINT("D=%d  H=%d  M=%d S=%d show\r\n",r_time.rtc_day,r_time.rtc_hour,r_time.rtc_min,r_time.rtc_sec);
	}
