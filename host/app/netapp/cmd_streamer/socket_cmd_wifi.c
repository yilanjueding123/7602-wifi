...
INT32S socket_cmd_wifi_start_server(socket_cmd_ctl_t *ctl_blk)
{
	...
	/* Create a new tcp connection handle */			//创建嵌套字
	...
	/* Bind the tcp connection to specific port */		//绑定端口
	...
	/* Listen for incoming connection */		//监听端口
...
INT32S socket_cmd_wifi_check_new_data(socket_cmd_ctl_t *ctl_blk)
{
	...
	err = netconn_recv_try(ctl_blk->socket_cmd_client, &pxRxBuffer);		//非阻塞方式
