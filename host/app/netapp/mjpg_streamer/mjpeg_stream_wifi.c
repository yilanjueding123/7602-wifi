...
#define WIFI_JPEG_Q_FINE_TUNE_ENABLE	0

#if WIFI_JPEG_Q_FINE_TUNE_ENABLE

INT32U g_total_frame = 0;
INT32U g_timeout_frame = 0;
INT32U g_total_txT = 0;

INT32U Frame_Time_Per_ms = 0;
INT32U Frame_TimeOut_Per_ms = 0;
INT32S WIFI_Q_bak = 0;
...
INT32S mjpeg_wifi_write_frame_data(mjpeg_ctl_t *ctl_blk)
{
	...
	cache_invalid_range(frame_info.addr, frame_info.size);  // 否則Wi-Fi會破圖
	s_t = OSTimeGet();
    ...
    else
    {
        ...
        g_total_txT+=s_t;
        g_total_frame++;
        if(s_t>8)		// 80ms   (12.5 fps)
        {
            g_timeout_frame++;
        }

        if (g_total_frame == 16)		//  statistics per about 1 seconds
        {
        	Frame_Time_Per_ms = (g_total_txT>>4);		// 一張需要多少 tick
        	Frame_TimeOut_Per_ms = g_timeout_frame;
            //printf("AVG_T=%d,%d/%d\r\n",Frame_Time_Per_ms,g_timeout_frame,g_total_frame);
            g_total_frame=0;
            g_timeout_frame=0;
            g_total_txT=0;
        }
...
// Note: 
// =============================================================================
INT8U mjpeg_wifi_Q_fine_tune(void)
{
#if WIFI_JPEG_Q_FINE_TUNE_ENABLE
	static INT32U FLAG = 0;
	INT32S WIFI_Q = 0;

	if (Frame_TimeOut_Per_ms)
	{
		if((Frame_TimeOut_Per_ms >= 4) && (Frame_TimeOut_Per_ms < 6))
		{
			WIFI_Q = 10;
		}
		else if(Frame_TimeOut_Per_ms >= 6)
		{
			WIFI_Q = 20;
		}
	}
	if (WIFI_Q > WIFI_Q_bak)
	{
		WIFI_Q_bak = WIFI_Q;
		FLAG = 0;
	}
	...
	// DBG_PRINT("%d ",WIFI_Q_bak);		
	Frame_TimeOut_Per_ms = 0;
	return (INT8U)WIFI_Q_bak;
